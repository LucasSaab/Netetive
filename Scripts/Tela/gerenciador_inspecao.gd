...
@onready var alvos_neutros: Array[ColorRect] = []


var posicao_do_clique: Vector2 = Vector2.ZERO

func _ready() -> void:
	alvos_neutros = [$Alvo2, $Alvo3]
...
func _on_botao_inspecionar_pressed() -> void:
	...
	if alvo_1 != null and mensagem_modal != null:
		...
		var _clicou_em_neutro: bool = false
		for alvo in alvos_neutros:
			if alvo.get_global_rect().has_point(posicao_do_clique):
				_clicou_em_neutro = true
				break
